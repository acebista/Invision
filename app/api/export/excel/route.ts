 ...
 * 
 * Exports approved invoices from a workspace as XLSX.
 * 
 * Request Body:
 * - workspaceId: UUID
 ...
 * This implementation returns a simple CSV with .xlsx extension as fallback.
 * For production, use the xlsx library to generate proper Excel files.
 */
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        // 5. Generate Excel using xlsx library (if available) or fallback to CSV
        // For this implementation, we'll attempt dynamic import of xlsx
        ...
        try {
            ...
            const worksheetData = [
                ['Invoice ID', 'Vendor Name', 'Invoice Number', 'Invoice Date (Raw)', 'Invoice Date (ISO)',
                    'Taxable Amount', 'VAT Amount', 'Grand Total', 'Currency', 'Status'],
                ...invoices.map(inv => [
                    inv.id,
                    inv.vendor_name_en || '',
            ...
            xlsxBuffer = XLSX.write(workbook, { type: 'buffer', bookType: 'xlsx' });

        } catch (e) {
            // xlsx not installed, return error with instructions
            return NextResponse.json({
                error: 'xlsx package not installed. Run: npm install xlsx',
                fallback: 'Use /api/export/csv instead'
            }, { status: 501 });
        }
        ...
        return new NextResponse(xlsxBuffer, {
            ...
            headers: {
                ...
                'Content-Disposition': `attachment; filename="invoices_${workspaceId}.xlsx"`
            }
        });
